...
class QueryAssetsUseCase:
    ...
    def execute(self,
        ...
        # 1. Fetch Data
        df = self.asset_data_source.fetch_data('MasterDataAsset')
        if df.empty: 
            return [{"status": "Database aset kosong."}]

        # 2. Normalisasi Nama Kolom DataFrame menjadi UPPERCASE
        df.columns = [str(col).strip().upper() for col in df.columns]

        # 3. Normalisasi Parameter Input & Mapping Alias
        ...
        # 4. Pre-processing Nilai Aset (Internal numeric column)
        ...
        # 5. Pre-processing Tanggal
        date_cols = ['TANGGAL INVENTORY', 'TANGGAL UPDATE']
        for col in date_cols:
            if col in df.columns:
                # Menangani format '27-Apr-2020', '2020-01-01', atau format Excel numeric
        ...
        # --- Filter No Asset (Logika Robust Anda) ---
        ...
        # --- Filter Kondisi (Smart Parsing Anda) ---
        ...
        # --- Filter Nilai Aset (Menggunakan kolom numeric internal) ---
        ...
        if task == 'get_top_per_group':
            if not group_by_field or not count_field:
                return [{"error": "Task 'get_top_per_group' membutuhkan group_by_field (misal: AREA) dan count_field (misal: NAMA ASET)."}]
        ...
        # 8. Sorting (Mendukung sort berdasarkan Tanggal atau Nilai)
        if sort_by:
            ...
            if actual_sort_col in df.columns:
                ...
                # Drop NaT (Tanggal rusak) agar tidak merusak urutan
                df = df.dropna(subset=[actual_sort_col])
                df = df.sort_values(by=actual_sort_col, ascending=is_asc)
        
        # JIKA tidak ada sort_by tapi ada filter tanggal, paksa urutkan terlama -> terbaru
        ...
        # Hapus kolom-kolom internal (yang diawali underscore) sebelum dikirim ke AI
